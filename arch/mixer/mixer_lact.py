...
class MixerLaCT(nn.Module):
    ...
    def forward(self, hidden_states, cache=None):
        ...
        if self.config.lact_rope:
            raise NotImplementedError("Rotary embeddings are not implemented in MixerLaCT yet.")
            """fast_q = rearrange(fast_q, '(b n_h) s d -> b s (n_h d)', n_h=self.n_heads)
            ...
            fast_q, fast_k = self.rotary(fast_q, fast_k, seqlen_offset=0, max_seqlen=max_seqlen, cu_seqlens=cu_seqlens)
        ...
        # [b * nh, s, d_ttt_head]
        if self.config.lact_prenorm:
            o = prenorm_block_causal_lact_swiglu(
                fw_w0, fw_w1, fw_w2,
                q, k, v,
