...
class Rotary(torch.nn.Module):
    ...
    def forward(self, x, start_pos=0):
        ...
        if total_len > self.seq_len_cached:
            self.seq_len_cached = max(
                2 * total_len, 16
            )  # each time we encounter a new seq_len, we cache the next 2*seq_len
...
class MixerAttention(nn.Module):
    def __init__(
        ...
        proj_dim = self.d_head * (
            self.n_heads + 2 * (0 if kv_share else self.n_kv_heads)
        )
        ...
        if self.rope:
            if self.swa:
                self.rotary = Rotary(
                    self.d_head, base=self.config.rope_theta_local
                )  # 477=3k/(2pi)
            else:
        ...
        if self.use_gate:
            ...
            if self.config.gate_act_attn == "silu":
                ...
                raise ValueError(
                    f"Unknown gate activation: {self.config.gate_act_attn}"
                )
    ...
    def forward(self, hidden_states, external_kv=None, cache=None):
        ...
        if self.scalable_softmax:
            ...
            log_pos = (
                torch.arange(start_pos + 1, start_pos + T + 1, device=q.device)
                .view(1, T, 1, 1)
                .float()
                .log()
            )
        ...
        k, v = repeat_kv(k, self.n_kv_repeats), repeat_kv(
            v, self.n_kv_repeats
        )  # GQA (todo: can be handled by FA)
...
class Attention(MixerAttention):
    def __init__(self, config: NanoConfig, swa: bool = False, kv_share: bool = False):
        ...
        self.c_proj = nn.Linear(
            self.expand_factor * self.d_model, self.d_model, bias=False
        )
...
class MixerDiffAttention(nn.Module):
    def __init__(
        ...
        lambdas_shape = (
            (self.n_heads // 2, head_dim) if config.full_lambdas else (head_dim,)
        )
        self.lambda_q1 = torch.nn.Parameter(
            torch.zeros(lambdas_shape, dtype=torch.float32).normal_(mean=0, std=0.1)
        )
        ...
        self.linear_qkv = nn.Linear(
            self.d_model,
            self.n_heads * self.head_dim + 2 * self.n_kv_heads * self.head_dim,
            bias=False,
        )
        if self.rope:
            if self.swa:
                self.rotary = Rotary(
                    self.head_dim, base=self.config.rope_theta_local
                )  # 477=3k/(2pi)
            else:
    ...
    def get_query_key_value_tensors(self, hidden_states, key_value_states=None):
        ...
        new_tensor_shape = mixed_qkv.size()[:-1] + (
            self.n_kv_heads,
            ((self.n_heads // self.n_kv_heads + 2) * self.head_dim),
        )
        mixed_qkv = mixed_qkv.view(*new_tensor_shape)
        ...
        value = value.reshape(
            value.size(0), value.size(1), value.size(2) // 2, 2 * value.size(3)
        )
    ...
    def forward(self, hidden_states, external_kv=None, cache=None):
        ...
        if self.scalable_softmax:
            ...
            pos = torch.arange(start_pos + 1, start_pos + T + 1, device=q.device).view(
                1, T, 1, 1
            )
            log_pos = (
                pos.float().log()
                if wsize <= 0
                else torch.clamp_max(pos.float(), wsize).log()
            )
            q = (self.softmax_scaler.view(1, 1, -1, 1) * log_pos) * q
        ...
        q1, q2 = (
            q[:, :, torch.arange(0, n_heads, 2)].contiguous(),
            q[:, :, torch.arange(1, n_heads, 2)].contiguous(),
        )
        k = k.view(B, L, n_kv_heads, d_head)
        k1, k2 = (
            k[:, :, torch.arange(0, n_kv_heads, 2)].contiguous(),
            k[:, :, torch.arange(1, n_kv_heads, 2)].contiguous(),
        )
        ...
        k1, k2, v = (
            repeat_kv(k1, self.n_kv_repeats),
            repeat_kv(k2, self.n_kv_repeats),
            repeat_kv(v, self.n_kv_repeats),
        )  # GQA
        ...
        lambda_full = (
            (lambda_1 - lambda_2 + self.lambda_init).view(1, 1, -1, 1).type_as(y1)
        )
...
class MixerGroupedTiedAttention(nn.Module):
        ...
        if self.rope:
            self.rotary = Rotary(
                ...
                base=(
                    ...
                    if self.swa
                    else self.config.rope_theta_global
                ),
            )
        if self.scalable_softmax:
            self.softmax_scaler = nn.Parameter(
    ...
    def forward(self, hidden_states, external_kv=None, cache=None):
        ...
        if external_kv is None:  # regular path
            qkv_proj = self.linear_qkv(x).view(
                B, T, -1, self.d_head
            )  # (B, T, H + h, D)
        ...
        q, q_rope = torch.split(
            q, [self.d1, self.d2], dim=-1
        )  # (B, T, H, d1), (B, T, H, d2)
        k_rope = (
            self.W_rope_k(x).unsqueeze(2).expand(-1, -1, self.n_heads, -1)
        )  # (B, T, H, d2)
        ...
        kv_tied, v = torch.split(
            kv, [self.d1, self.d2], dim=-1
        )  # (B, T, h, d1), (B, T, h, d2)
        kv_tied, v = repeat_kv(kv_tied, self.n_kv_repeats), repeat_kv(
            v, self.n_kv_repeats
        )
...
class MixerGroupedTiedDifferentialAttention(nn.Module):
    def __init__(
        ...
        self.linear_qkv = nn.Linear(
            self.d_model, total_heads * self.head_dim, bias=False
        )
        self.W_rope_k = nn.Linear(self.d_model, self.d2)
        ...
        if self.rope:
            self.rotary = Rotary(
                self.d2,
                base=config.rope_theta_local if swa else config.rope_theta_global,
            )
        if self.scalable_softmax:
            self.softmax_scaler = nn.Parameter(
                torch.ones(self.n_heads, dtype=torch.float32)
        ...
        if self.use_gate:
            if config.gate_type_attn == "elementwise":
                ...
                self.g_proj = nn.Linear(
                    ...
                    (
                        self.n_heads // 2
                        if config.gate_type_attn == "headwise"
                        else self.n_heads
                    ),
                    bias=False,
                )
        ...
        lambdas_shape = (
            (self.n_heads // 2, self.d1) if config.full_lambdas else (self.d1,)
        )
    ...
    def forward(self, hidden_states, external_kv=None, cache=None):
        ...
        k_rope = (
            self.W_rope_k(hidden_states).unsqueeze(2).expand(-1, -1, self.n_heads, -1)
        )
        if self.rope:
            cos, sin = self.rotary(q_rope, start_pos)
            q_rope = apply_rotary_emb(q_rope, cos, sin)
...
def repeat_kv(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
    ...
    hidden_states = hidden_states[:, :, :, None, :].expand(
        batch, seqlen, num_key_value_heads, n_rep, head_dim
    )
