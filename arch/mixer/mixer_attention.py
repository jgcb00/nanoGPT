...
class MixerAttention(nn.Module):
    def __init__(self, config: NanoConfig, swa: bool = False, kv_share: bool = False, layer_depth: int = 0):
        ...
        self.linear_qkv = ScaledLinear(config, self.d_model, proj_dim, bias=False)
...
class Attention(MixerAttention):
    def __init__(self, config: NanoConfig, swa: bool = False, kv_share: bool = False):
        ...
        # output projection
        self.c_proj = ScaledLinear(config, self.expand_factor * self.d_model, self.d_model, bias=False)
...
class MixerDiffAttention(nn.Module):
    def __init__(self, config: NanoConfig, swa: bool = False, kv_share: bool = False, layer_depth: int = 0):
        ...
        assert self.d_model % self.n_heads == 0
        self.linear_qkv = ScaledLinear(config, self.d_model, self.n_heads*self.head_dim + 2*self.n_kv_heads*self.head_dim, bias=False)
        if self.rope:
            if self.swa:
        ...
        if self.use_gate:
            ...
            if self.config.gate_type_attn == "elementwise":
                ...
                self.g_proj = ScaledLinear(config, self.d_model, self.n_heads//2, bias=False)
            else:
                raise ValueError(f"Unknown gate type: {self.config.gate_type_attn}")
...
class MixerGroupedTiedAttention(nn.Module):
    def __init__(self, config: NanoConfig, swa: bool = False, kv_share: bool = False, layer_depth: int = 0):
        ...
        self.linear_qkv = ScaledLinear(config, self.d_model, n_heads * self.d_head, bias=False)
        self.W_rope_k = ScaledLinear(config, self.d_model, self.d2)
        if self.qk_norm:
...
class MixerGroupedTiedDifferentialAttention(nn.Module):
    def __init__(self, config: NanoConfig, swa: bool = False, kv_share: bool = False, layer_depth: int = 0):
        ...
        self.linear_qkv = ScaledLinear(config, self.d_model, total_heads * self.head_dim, bias=False)
        self.W_rope_k = ScaledLinear(config, self.d_model, self.d2)
        ...
        if self.use_gate:
            if config.gate_type_attn == "elementwise":
                ...
                self.g_proj = ScaledLinear(config, self.d_model, self.n_heads // 2 if config.gate_type_attn == "headwise" else self.n_heads, bias=False)
