            ...
            for p in group["params"]:
                ...
                if len(state) == 0:  # Initialization
                    ...
                    if len(p.shape) == 2:
                        state["sides"] = [
                            (
                                torch.zeros((d, d), device=p.device, dtype=dtype)
                                if d < group["max_dim"]
                                else None
                            )
                            for d in p.shape
                        ...
                        state["q_sides"] = [
                            (
                                torch.eye(d, device=p.device, dtype=dtype)
                                if d < group["max_dim"]
                                else None
                            )
                            for d in p.shape
                ...
                if len(p.shape) != 2 or (
                    p.shape[0] > group["max_dim"]
                    or p.shape[1] > group["max_dim"]
                    or p.shape[0] < group["d_model"]
                    or p.shape[1] < group["d_model"]
                ):
                ...
                if len(p.shape) == 2 and (
                    p.shape[0] <= group["max_dim"]
                    and p.shape[1] <= group["max_dim"]
                    and p.shape[0] >= group["d_model"]
                    and p.shape[1] >= group["d_model"]
                ):
                    m = (
                        state["q_sides"][0].T @ m
