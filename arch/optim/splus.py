# from https://github.com/kvfrans/splus
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if len(state) == 0: # Initialization
                    ...
                    group['train_mode'] = True
                    if len(p.shape) == 2:
                        state['sides'] = [torch.zeros((d, d), device=p.device, dtype=dtype) if d < group['max_dim'] else None for d in p.shape]
                        state['q_sides'] = [torch.eye(d, device=p.device, dtype=dtype) if d < group['max_dim'] else None for d in p.shape]

                # Shape-dependent scaling
                if len(p.shape) != 2 or (p.shape[0] > group['max_dim'] or p.shape[1] > group['max_dim'] or p.shape[0] < group['d_model'] or p.shape[1] < group['d_model']):
                ...
                if len(p.shape) == 2 and (p.shape[0] <= group['max_dim'] and p.shape[1] <= group['max_dim'] and p.shape[0] >= group['d_model'] and p.shape[1] >= group['d_model']):
                    ...
                    if state['step'] == 1 or state['step'] % group['inverse_every'] == 0:
                        if state['sides'][0] is not None:
                            S = state['sides'][0].float()
                            eye = torch.eye(S.size(0), device=p.device)
                            _, eigvecs = torch.linalg.eigh(S + group['eps'] * eye)
                            state['q_sides'][0] = eigvecs.to(dtype)
                        if state['sides'][1] is not None:
                            S = state['sides'][1].float()
                            eye = torch.eye(S.size(0), device=p.device)
                            _, eigvecs = torch.linalg.eigh(S + group['eps'] * eye)
                            state['q_sides'][1] = eigvecs.to(dtype)
                else:
                    u = torch.sign(m)
