...
def must_be_in_open_interval(
    name: str, value: float | Tensor, low: float, high: float
) -> None:
    if not low < value < high:
        raise ValueError(f"{name} must be in ({low}, {high})")


def must_be_in_closed_interval(
    name: str, value: float | Tensor, low: float, high: float
) -> None:
    if not low <= value <= high:
        raise ValueError(f"{name} must be in [{low}, {high}]")
...
def must_be_in_left_open_interval(
    name: str, value: float | Tensor, low: float, high: float
) -> None:
    if not low < value <= high:
        raise ValueError(f"{name} must be in ({low}, {high}]")
...
def must_be_in_right_open_interval(
    name: str, value: float | Tensor, low: float, high: float
) -> None:
    if not low <= value < high:
        raise ValueError(f"{name} must be in [{low}, {high})")
        ...
        match m:
            case _ConvNd():
                ...
                if m.transposed:
                    ...
                    setattr(
                        m.weight,
                        "parameter_info",
                        ParameterInfo(Size([m.groups, out_features, in_features])),
                    )
            case MultiheadAttention():
                if m.in_proj_weight is not None:
                    ...
                    setattr(
                        m.in_proj_weight,
                        "parameter_info",
                        ParameterInfo(Size([3, out_features, in_features])),
                    )
            case Bilinear():
            ...
            case Embedding():
                ...
                setattr(
                    m.weight,
                    "parameter_info",
                    ParameterInfo(Size([num_groups, out_features, 1])),
                )
            case EmbeddingBag():
...
def whitening(
    G: Tensor, beta: float, steps: int, diag: bool, eps: float = 1e-8
) -> Tensor:
    # [m, n] m <= n
    assert G.ndim >= 2
