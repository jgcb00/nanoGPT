...
def eval_pg19(log_dir, model, nsamples, ctx_len, batch_size, log_wandb=True):
    ...
    for example in tqdm(ds["train"], total=nsamples):
        ...
        if len(batch_examples) == batch_size:
            ...
            x = torch.tensor(
                [ex[:-1] for ex in batch_examples], dtype=torch.long, device="cuda"
            )
            y = torch.tensor(
                [ex[1:] for ex in batch_examples], dtype=torch.long, device="cuda"
            )
    ...
    if log_wandb:
        ...
        wandb.log(
            {
                "eval/per_token_loss_plot": wandb.plot.line(
                    table, "token_position", "loss", title="Loss per Token"
                )
            },
...
if __name__ == "__main__":
    ...
    class Args:
        ...
        def __post_init__(self):
            assert (
                self.run_dir.exists()
            ), f"Run directory {self.run_dir} does not exist."
