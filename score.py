...
train_loader = DistributedDataLoader(config.input_bin, args.batch_size, config.sequence_length, ddp_rank, ddp_world_size)
...
if master_process:
    pbar = tqdm(total=int(total_shards*(tokens_per_shard / (args.batch_size * config.sequence_length))))
