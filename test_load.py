...
print(
    f"Max: {scores_np.max()}, Min: {scores_np.min()}, Mean: {scores_np.mean()}, Std: {scores_np.std()}"
)
...
smooth_weights_np = (
    smooth_weights.cpu().numpy()
    if hasattr(smooth_weights, "cpu")
    else np.array(smooth_weights)
)
...
plt.figure(figsize=(12, 6))
plt.plot(
    smooth_weights_np[0, :], marker="o", linestyle="-", label="Smooth Weight Evolution"
)
