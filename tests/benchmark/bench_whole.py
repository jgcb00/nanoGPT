...
def benchmark(T, provider):
    ...
    for _ in range(10):
        x = torch.randn(B, T, config.d_model, requires_grad=requires_grad, dtype=dtype, device=device)
    ...
    if not 'bwd' in provider:
        ...
        with ctx:
            results = triton.testing.do_bench(lambda: f(x).backward(do), quantiles=quantiles)
